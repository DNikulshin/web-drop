  ...
  return {
    send: async (command: any) => {
      ...
      if (name === 'PutObjectCommand') {
        ...
        store.set(Key, { buffer: Buffer.from(Body), meta: command.input.Metadata || {} });
        return {};
      }
...
describe('S3 + WebSocket E2E', () => {
  ...
  it('S3 upload -> download -> cleanup', async () => {
    ...
    const payload = { filename: 's3.txt', content: Buffer.from('s3-content').toString('base64'), ttlSeconds: 1 };
  ...
  it('WebSocket session sync (E2E)', async () => {
    ...
    const createRes = await server.inject({ method: 'POST', url: '/api/sessions', payload: {} });
    ...
    const msg = await new Promise((resolve, reject) => {
      ...
      ws.on('open', () => {
        ws.send(JSON.stringify({ type: 'session.text.update', data: 'hello e2e' }));
      });
      ws.on('message', (data) => {
